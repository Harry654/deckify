...
const MyFlashcards = () => {
  ...
  const [selectedDeck, setSelectedDeck] = useState<TSelectedDeck | null>(null);
  const { userInfo } = useAuth();

  async function selectFlashcardset(name: string) {
    setDeckLoading(true);
    const flashcardsetRef = doc(
      db,
      `users/${userInfo?.id}/flashcardSets/${name}`,
    );
    ...
    if (docSnap.exists()) {
      ...
      setSelectedDeck({ name: name, data: docSnap.data().flashcards });
    } else {
          ...
          <div className="w-full px-4 lg:w-8/12">
            {selectedDeck ? (
              <div>
                    ...
                    <div className="mb-5 mr-10 flex items-center">
                      ...
                      <div className="w-full">
                        <span className="mb-1 text-base font-medium text-body-color">
                          By <span> {userInfo?.firstName}</span>
                        </span>
                      </div>
                    </div>
                ...
                <Grid container>
                  {selectedDeck.data.map((flashcard, index) => (
                    <div
                      key={index}
              ...
              <>
                {deckLoading ? (
                  ...
                  <p className="mr-5 flex items-center text-base font-medium text-body-color">
                    Select a deck to display cards
                  </p>
                )}
              </>
            )}
                  ...
                  .map((card, index) => (
                    ...
                    >
                      <SingleDeck
                        ...
                        slug="#"
                        date="12 Feb 2025"
                        handleClicked={() => selectFlashcardset(card.name)}
                      />
                    </li>
                  ))}
              </ul>
